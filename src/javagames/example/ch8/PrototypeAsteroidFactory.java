...
public class PrototypeAsteroidFactory {
    ...
    public PrototypeAsteroid createLargeAsteroid(Vector2f position) {
        ...
        asteroid.setPolygon(getRandomPolygon(LARGE));
    ...
    public PrototypeAsteroid createMediumAsteroid(Vector2f position) {
        PrototypeAsteroid asteroid = new PrototypeAsteroid(wrapper);
        asteroid.setPosition(position);
        asteroid.setPolygon(getRandomPolygon(MEDIUM));
    ...
    public PrototypeAsteroid createSmallAsteroid(Vector2f position) {
        PrototypeAsteroid asteroid = new PrototypeAsteroid(wrapper);
        asteroid.setPosition(position);
        asteroid.setPolygon(getRandomPolygon(SMALL));
        asteroid.setSize(Size.Small);
        return asteroid;
    }

    private Vector2f[] mirror(Vector2f[] poly) {
        float x = rand.nextBoolean() ? -1 : 1;
        float y = rand.nextBoolean() ? -1 : 1;
        Matrix3x3f mat = Matrix3x3f.scale(x, y);
        return Utility.transform(poly, mat);
    }

    private Vector2f[] getRandomPolygon(Vector2f[][] polygons) {
        return mirror(polygons[rand.nextInt(polygons.length)]);
    }
}
